...
def main():
    ...
    global sessionCode, sessionID, trasnslateObjects, connection
    ...
    if len(session) == 0:
        ...
        wordsDict = {
            0: {'word': 'pencil', 'tranlations': json.dumps(['карандаш', 'кисть'], ensure_ascii=False),
                'status': 0},
            1: {'word': 'table', 'tranlations': json.dumps(['стол', 'таблица'], ensure_ascii=False), 'status': 0},
            2: {'word': 'girl', 'tranlations': json.dumps(['девушка', 'девочка'], ensure_ascii=False), 'status': 0},
            3: {'word': 'down', 'tranlations': json.dumps(['вниз', 'внизу'], ensure_ascii=False), 'status': 0},
            4: {'word': 'top', 'tranlations': json.dumps(['верх', 'верхний'], ensure_ascii=False), 'status': 0},
            5: {'word': 'read', 'tranlations': json.dumps(['читать', 'прочесть'], ensure_ascii=False), 'status': 0},
            6: {'word': 'help', 'tranlations': json.dumps(['помощь', 'помогать'], ensure_ascii=False), 'status': 0},
            7: {'word': 'love', 'tranlations': json.dumps(['любовь', 'любить'], ensure_ascii=False), 'status': 0},
            8: {'word': 'body', 'tranlations': json.dumps(['тело', ' корпус'], ensure_ascii=False), 'status': 0},
            9: {'word': 'go', 'tranlations': json.dumps(['идти', 'ехать'], ensure_ascii=False), 'status': 0},
            10: {'word': 'people', 'tranlations': json.dumps(['люди', 'народ'], ensure_ascii=False), 'status': 0},
            11: {'word': 'picture', 'tranlations': json.dumps(['картинка'], ensure_ascii=False), 'status': 0},
            12: {'word': 'again', 'tranlations': json.dumps(['снова'], ensure_ascii=False), 'status': 0},
            13: {'word': 'animal', 'tranlations': json.dumps(['животное'], ensure_ascii=False), 'status': 0},
            14: {'word': 'mother', 'tranlations': json.dumps(['мать'], ensure_ascii=False), 'status': 0},
            15: {'word': 'place', 'tranlations': json.dumps(['место'], ensure_ascii=False), 'status': 0},
            16: {'word': 'big', 'tranlations': json.dumps(['большой'], ensure_ascii=False), 'status': 0},
            17: {'word': 'here', 'tranlations': json.dumps(['здесь'], ensure_ascii=False), 'status': 0},
            18: {'word': 'land', 'tranlations': json.dumps(['земля'], ensure_ascii=False), 'status': 0},
            19: {'word': 'end', 'tranlations': json.dumps(['окончание'], ensure_ascii=False), 'status': 0},
            20: {'word': 'small', 'tranlations': json.dumps(['маленький'], ensure_ascii=False), 'status': 0},
            21: {'word': 'play', 'tranlations': json.dumps(['играть'], ensure_ascii=False), 'status': 0}
        }
...
def getWords():
    ...
    selectQuery = "SELECT * FROM words WHERE status = 2 AND id_session = " + str(sessionID) + " LIMIT " + str(
        LIMIT_WORDS - lenInCorrectWords - 1)
...
def getWord():
    global trasnslateObjects, sessionID, sessionsLimit
    ...
    if len(trasnslateObjects[sessionID].words) > 1:
        if trasnslateObjects[sessionID].isStart:
            trasnslateObjects[sessionID].nextWord = trasnslateObjects[sessionID].words[1][1]
            trasnslateObjects[sessionID].isStart = False
        else:
            trasnslateObjects[sessionID].nextWord = trasnslateObjects[sessionID].words[1][1]
    else:
    ...
    #logging.error('%s words',  trasnslateObjects[sessionID].curWord)
    #logging.error('%s words', trasnslateObjects[sessionID].nextWord)
    ...
    if len(trasnslateObjects[sessionID].words) == 0:
        trasnslateObjects[sessionID].isNextWord = True
            ...
            if (trasnslateObjects[sessionID].cntWords == sessionsLimit[sessionID]) or (trasnslateObjects[sessionID].isRestart):
                ...
                if requestText == "нет":
                    ...
                    if len(trasnslateObjects[sessionID].words) == 0:
                        ...
                        return
                    trasnslateObjects[sessionID].isStart = True
                    listWord = getWord()
                    trasnslateObjects[sessionID].idCurWord = listWord[0]
                    trasnslateObjects[sessionID].nextWord = listWord[1]
                    trasnslateObjects[sessionID].translationOptions = listWord[2]
                    if trasnslateObjects[sessionID].isNextWord:
                        res['response']['text'] = "Продолжаем.\nПереведи слово - " + trasnslateObjects[sessionID].curWord
                    else:
                        res['response']['text'] = "Продолжаем.\nПереведи слово - " + trasnslateObjects[sessionID].nextWord
